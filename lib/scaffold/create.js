...
var BASE_PACKAGE = {
  ...
  readme: 'README.md',
  dependencies: {
    'richcore-lib': '^' + bitcore.version,
    'richcore-node': version
  }
};
...
function createConfigDirectory(options, configDir, isGlobal, done) {
  mkdirp(configDir, function(err) {
    ...
    try {
      fs.writeFileSync(configDir + '/richcore-node.json', configJSON);
      if (!isGlobal) {
 ...
 * @param {String} options.dirname - The name of the richcore node configuration directory
...
function create(options, done) {
  ...
  async.series([
    function(next) {
      // Setup the the richcore-node directory and configuration
